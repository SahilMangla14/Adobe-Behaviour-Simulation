...
class CustomDataset(Dataset):
    def __init__(self, tweet_date, tweet_content, tweet_username, tweet_media, tweet_company, likes):
    ...
    def __getitem__(self, idx):
        ...
        date_inputs = tokenizer(date, return_tensors="pt", truncation=True, padding=True, max_length=128)
        date_input_ids = date_inputs['input_ids'].squeeze()
        date_attention_mask = date_inputs['attention_mask'].squeeze()

        content_inputs = tokenizer(content, return_tensors="pt", truncation=True, padding=True, max_length=128)
        ...
        username_inputs = tokenizer(username, return_tensors="pt", truncation=True, padding=True, max_length=128)
        ...
        company_inputs = tokenizer(company, return_tensors="pt", truncation=True, padding=True, max_length=128)
        company_input_ids = company_inputs['input_ids'].squeeze()
        company_attention_mask = company_inputs['attention_mask'].squeeze()


        return date_input_ids, date_attention_mask , content_input_ids, content_attention_mask, username_input_ids, username_attention_mask, media_input_ids, media_attention_mask, company_input_ids, company_attention_mask, like
